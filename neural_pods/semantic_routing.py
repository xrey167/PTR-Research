...
class SemanticRouter:
    ...
    def index(self, generation_key, artifact_key, principal="local"):
        ...
        payload = {"knowledge_key": node["knowledge_key"], "generation_key": generation_key,
                   ...
                   # Measurements and arithmetic constraints route through the
                   # typed math path; prose relations remain context Pods.
                   "semantic_type": "math" if s["predicate"] in {"lead_time", "deadline", "buffer"} else "context"}
    ...
    def select(self, question, principal="local", filters=None, learned=True):
        ...
        filters = filters or {}
        if not set(filters) <= {"tags", "domain", "language", "type", "semantic_type"}: raise ValueError("Unsupported retrieval filter")
        ...
        for item in items:
            ...
            if filters.get("semantic_type") and ("math" if s["predicate"] in {"lead_time", "deadline", "buffer"} else "context") != filters["semantic_type"]: continue
        ...
        for field in ["type", "domain", "language", "semantic_type"]:
