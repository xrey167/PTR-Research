...
## Latest infrastructure recheck

A fresh read-only SSH probe on 2026-09-16 confirms that the state is
unchanged: `/dev/kvm` is present, but Docker, standalone `qemu-system-x86_64`,
`amd-smi`/`rocm-smi`, and `/dev/kfd` are absent. The host exposes NVIDIA GPUs,
not AMD Instinct devices. No official benchmark or Hyperloom launch was
started from this host.
