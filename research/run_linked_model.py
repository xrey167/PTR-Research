...
def main():
    ...
    p.add_argument('--model-manifest',type=Path,
                   help='Original model manifest for a completed identity-transfer source')
    args=p.parse_args()
    run=args.run.resolve()
    started=time.perf_counter()
    if args.stage=='links':
        run.mkdir(parents=True,exist_ok=False)
        source_report=json.loads((args.source/'report.json').read_text(encoding='utf-8'))
        assert source_report['status']=='completed'
        identity_source='predictions' in source_report and source_report.get('representation_unchanged') is True
        if identity_source:
            if args.model_manifest is None:raise ValueError('Identity source requires --model-manifest')
            manifest=json.loads(args.model_manifest.read_text(encoding='utf-8'))
            manifest['router_kind']='identity-dragonfly'
            manifest['model_manifest_source']=str(args.model_manifest.resolve())
            manifest['questions']=list(dict.fromkeys(r['question'] for r in source_report['predictions'] if r['stage']=='after'))
            manifest['transfer_source_sha256']=hashlib.sha256((args.source/'report.json').read_bytes()).hexdigest()
        else:
    ...
    try:
        ...
        router_type=IdentityDragonfly if manifest.get('router_kind')=='identity-dragonfly' else DragonflyRouter
        router=router_type(reg,encoder,run,encoder_id=info['repo']+'@'+info['revision'])
        router.load_weights()
        ...
        if args.stage=='links':
            ...
            for row in proofs:
                ...
                reader_text=question+'\nReply with only the integer number of days.'+tails[generation]
                # The fact is available only through the activated prefix KV
                # state. It must never be repeated in the reader continuation.
                assert fact not in reader_text
                suffix=tok.encode(reader_text,add_special_tokens=False,return_tensors='pt')
                ...
                item={'question':question,'text':text,'expected':expected,'correct':text==expected and output['eos'],
                      ...
                      'activation_mode':'prefix_kv_only','fact_in_reader_suffix':False}
