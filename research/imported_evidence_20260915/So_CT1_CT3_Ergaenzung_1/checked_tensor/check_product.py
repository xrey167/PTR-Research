...
def verify_product(t,output,rng):
    ...
    if set(t)!=set(('B','S','R','C','Y','X')) or any(not isinstance(a,np.ndarray) or a.ndim!=2 for a in t.values()):
        return False
    m,d=t['B'].shape;k=t['C'].shape[1]
    if (m,d,k) not in ((2,4,2),(3,8,3),(4,16,4)):
        return False
    shapes={'B':(m,d),'S':(m,d),'R':(m,d),'C':(m,k),'Y':(k,d),'X':(k,d)}
    if any(t[n].shape!=s for n,s in shapes.items()) or not isinstance(output,np.ndarray) or output.shape!=(m,d):
        return False
    vals={}
    for n,a in t.items():
        scaled=a*16
