...
def main():
    ...
    out = root / "runs" / "topic-model-pod-003"
    ...
    adapter = root / "runs" / "topic-lora-gpu-001" / "adapter"
    ...
    report = {"status": "linked_existing_adapter_topic_generation", "source": source,
              "generation": generation, "pod_artifact": pod,
              "pod_identity": "pod:main-model-typed-topics", "adapter_path": str(adapter),
              "adapter_sha256": adapter_hash, "training_report": "runs/topic-lora-gpu-001/report.json",
