...
def main():
    ...
    try:
        ...
        for row in rows:
            ...
            # Keep the persisted diagnostic tensor on CPU.  Holding a CUDA
            # tensor in ``raw`` across hundreds of cases can trigger allocator
            # pressure and a late CUDA launch failure when safetensors saves.
            raw[row['id']] = result['logits'].detach().to('cpu')
