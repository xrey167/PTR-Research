...
def main():
    ...
    shutil.copyfile(rows_path, output / 'cases.json')
    sources = {}
    for name in ('evaluate_reader.py', 'train_reader.py', 'reader_prompt.py',
    ...
    state = {'status': 'running', 'phase': 'verify_model', 'rows': [],
             ...
             'cases_sha256': file_sha(rows_path), 'sources': sources,
             ...
             'attention_implementation': 'eager', 'precision': 'bfloat16', 'device': 'cpu',
    ...
    try:
        ...
        for row in rows:
            ...
            suffix_ids = tokenizer.encode(suffix, add_special_tokens=False, return_tensors='pt')
            ...
            state['rows'].append({'id': row['id'], 'text': answer, 'tokens': result['tokens'],
