...
def load_bundle(path):
    ...
    if (training['micro_batch_size'], training['world_size']) != (1, 1):
        raise ValueError('Only single-process microbatch one is supported')
    ...
    if training['effective_batch_size'] != training['gradient_accumulation_steps']:
        raise ValueError('Effective batch mismatch')
    rows = json.loads((path / 'inputs/train.json').read_text(encoding='utf-8'))
    steps = math.ceil(len(rows) / training['gradient_accumulation_steps']) * training['epochs']
...
def train_windows(model, encoded, training, report_step, preflight=False):
    ...
    total = math.ceil(len(encoded) / training['gradient_accumulation_steps']) * training['epochs']
    ...
    for epoch in range(training['epochs']):
        ...
        if preflight:
            # Exercise the largest sequence repeatedly through a full window,
            # then a second optimizer step with Adam state already allocated.
            longest = max(indices, key=lambda i: len(encoded[i]['input_ids']))
            indices = [longest] * (2 * training['gradient_accumulation_steps'])
        for window in accumulation_windows(indices, training['gradient_accumulation_steps']):
            batches = [{k: torch.tensor([v], dtype=torch.long, device=device)
                        for k, v in encoded[i].items()} for i in window]
...
def main():
    ...
    try:
        ...
        if state['adapter_parameters_before'] == state['adapter_parameters_after']:
        ...
        if args.mode == 'train':
            ...
            state['limitations'] = 'Two optimizer steps on the longest training row; available memory and paging can change. No quality or runtime lineage claim.'
        state['phase'] = 'done'
        save()
