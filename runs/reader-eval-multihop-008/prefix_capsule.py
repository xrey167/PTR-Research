...
def weights_hash(model):
    h = hashlib.sha256()
    def raw(tensor):
        # Buffer view avoids a full-size extra bytes copy of large embeddings.
        array = tensor.detach().cpu().contiguous().reshape(-1).view(torch.uint8).numpy()
        h.update(memoryview(array).cast('B'))
    for name, tensor in model.named_parameters():
        h.update(name.encode())
        raw(tensor)
    from torch.ao.nn.quantized.dynamic import Linear as Int8Linear
    # Packed quantized weights are not in named_parameters: explicitly cover them.
    for name, module in model.named_modules():
        if isinstance(module, Int8Linear):
            weight = module.weight()
            h.update((name + ':dynamic-int8:' + str(weight.shape) + ':' + str(weight.qscheme())).encode())
            raw(weight.int_repr())
            if weight.qscheme() in (torch.per_tensor_affine, torch.per_tensor_symmetric):
                h.update(struct.pack('!dq', weight.q_scale(), weight.q_zero_point()))
            else:
                raw(weight.q_per_channel_scales())
                raw(weight.q_per_channel_zero_points())
                h.update(str(weight.q_per_channel_axis()).encode())
            bias = module.bias()
...
class PrefixCapsules:
    ...
    def decode(self, suffix_ids, cache, eos_ids, max_tokens=12):
        ...
        out = self.model(input_ids=suffix_ids,
                         attention_mask=torch.ones((1, prefix_length + suffix_ids.shape[1]), dtype=torch.long, device=suffix_ids.device),
        ...
        for _ in range(max_tokens):
            ...
            out = self.model(input_ids=torch.tensor([[token]], device=suffix_ids.device),
                             attention_mask=torch.ones((1, length), dtype=torch.long, device=suffix_ids.device),
