...
def main():
    ...
    p.add_argument('--quantization', choices=['none','dynamic_int8'], default='none')
    a = p.parse_args()
    a.output.mkdir(parents=True, exist_ok=False)
    protocol = {'schema': 'local-prefix-interface:v1', 'entities': ['Neral', 'Vost'],
                'model_path':str(a.model.resolve()),
                'model_dtype':a.dtype, 'recorded_logits_dtype':'float32',
    ...
    download_manifest = a.model / 'download-manifest.json'
    if download_manifest.exists():
        model_manifest = json.loads(download_manifest.read_text(encoding='utf-8'))
        if model_manifest['status'] != 'verified':
            raise ValueError('Model download has not been verified')
        protocol['model_download_manifest'] = model_manifest
    protocol['source_hashes'] = {p.name:hashlib.sha256(p.read_bytes()).hexdigest()
        for p in [Path(__file__), Path(__file__).with_name('prefix_capsule.py')]}
    if a.quantization == 'dynamic_int8':
        quantization_source = Path(__file__).with_name('cpu_int8.py')
        protocol['source_hashes'][quantization_source.name] = hashlib.sha256(quantization_source.read_bytes()).hexdigest()
    ...
    def save():
        if raw_logits:
            temporary_logits = a.output / 'first_logits.safetensors.tmp'
            save_file(raw_logits, temporary_logits)
            os.replace(temporary_logits, a.output / 'first_logits.safetensors')
        result['elapsed_s'] = time.perf_counter() - start
        memory = psutil.Process().memory_info()
        result['resources'] = {'rss_bytes':memory.rss,'private_bytes':getattr(memory,'private',None),
                               'page_faults':getattr(memory,'num_page_faults',None),
                               'available_ram_bytes':psutil.virtual_memory().available}
        temporary_results = a.output / 'results.json.tmp'
        temporary_results.write_text(json.dumps(result, indent=2), encoding='utf-8')
        os.replace(temporary_results, a.output / 'results.json')
    save()
    ...
    try:
        ...
        model = AutoModelForCausalLM.from_pretrained(a.model, local_files_only=True, trust_remote_code=False,
                     dtype=getattr(torch,a.dtype), attn_implementation='sdpa').eval()
        model.requires_grad_(False)
        if a.quantization == 'dynamic_int8':
            from research.cpu_int8 import quantize_in_place
            result['quantization'] = quantize_in_place(model, lambda s: print(json.dumps({'event':'quantize',**s}),flush=True))
            save()
        ...
        for entity in protocol['entities']:
            for vi, value in enumerate(protocol['values']):
                ...
                for op in protocol['operators']:
                    ...
                    save()
                    print(json.dumps({'event':'checkpoint','completed_sequences':len(result['rows']),
