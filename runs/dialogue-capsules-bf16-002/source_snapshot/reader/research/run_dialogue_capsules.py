...
def main():
    ...
    p.add_argument('--resume-from',type=Path,help='Continue verified completed rows of an explicitly failed run in a new directory')
    a=p.parse_args()
    if a.copy_from and a.resume_from:raise ValueError('Choose copy or resume, not both')
    resume=None
    if a.resume_from:
        if a.stage!='reader':raise ValueError('Only reader continuation is supported')
        resume=json.loads((a.resume_from/'failure.json').read_text(encoding='utf-8'))
        assert resume['status']=='failed'
        assert not psutil.pid_exists(resume['pid']), 'Source process is still present'
        assert resume['reader_precision']==a.reader_precision and resume['reader_input_format']==a.reader_input
        a.copy_from=a.resume_from
    ...
    if a.copy_from:
        ...
        if resume:
            shutil.copyfile(a.resume_from/'failure.json',run/'resume-source-report.json')
            shutil.copyfile(a.resume_from/'reader-logits.safetensors',run/'resume-source-logits.safetensors')
            shutil.copytree(a.resume_from/'source_snapshot',run/'resume-source-snapshot')
    if a.stage=='links':
        run.mkdir(parents=True,exist_ok=False)
    ...
    if resume:
        report.update(resumed_from=str(a.resume_from.resolve()),
            resumed_rows=[row['id'] for row in resume['rows']],
            resume_source_sha256=hashlib.sha256((run/'resume-source-report.json').read_bytes()).hexdigest(),
            resumed_source_final_weight_hash_available=False)
    ...
    def save():
        ...
        write_progress(run/(a.stage+'.json'),report)
    ...
    try:
        ...
        if a.stage=='links':
            ...
            inherited_raw=verify_prefix(a.resume_from,resume,linked,r,tok,eos) if resume else {}
            ...
            if resume:assert base==resume['reader_model_sha256']
            def heartbeat(module,args,result):
                report['forward_calls']+=1
                try:save()
                except PermissionError:
                    # Progress reporting must not discard a model forward on a temporary read lock.
            ...
            capsules=PrefixCapsules(r,model,run/'capsule_states',base);joined=LinkedCapsules(router,capsules);raw=dict(inherited_raw)
            compiled=dict(resume.get('compiled_capsules',{})) if resume else {}
            if resume:
                for generation,key in compiled.items():joined.bind_variant(generation,key)
                report['compiled_capsules']=compiled;report['rows']=list(resume['rows'])
                save_file(raw,run/'reader-logits.safetensors');save()
            for row in linked['rows']:
                if resume and row['id'] in report['resumed_rows']:continue
