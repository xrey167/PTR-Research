...
def main():
    p=argparse.ArgumentParser();p.add_argument('stage',choices=['links','reader']);p.add_argument('--run',type=Path,required=True);a=p.parse_args()
    ...
    try:
        ...
        if a.stage=='links':
            ...
            for row in linked['rows']:
                ...
                s=r.node(prepared.generation_key)['payload']['semantic'];aliases=s['retrieval']['trusted_aliases']
